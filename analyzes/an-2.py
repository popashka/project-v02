...
#pyplot.ylim(ymax = 0.1, ymin = 0)
#pyplot.title('Eth Spread')
#pyplot.legend()
#pyplot.show()
...
for i in range(df.shape[0]):
    ...
    if (cnt == 5):
        if (l1 != 0 and ad1[len(ad1) - 1] != 0 and math.log(ad1[len(ad1) - 1]) != 0):
            mec1.append(math.log(df.iat[i, 2] / l1) / (4 * math.log(ad1[len(ad1) - 1])))
        else:
            mec1.append(1)

        if (l2 != 0 and ad2[len(ad2) - 1] != 0 and math.log(ad2[len(ad2) - 1]) != 0):
            mec2.append(math.log(df.iat[i, 5] / l2) / (4 * math.log(ad2[len(ad1) - 1])))
        else:
            mec2.append(1)

        if (l3 != 0 and ad3[len(ad3) - 1] != 0 and math.log(ad3[len(ad3) - 1]) != 0):
            mec3.append(math.log(df.iat[i, 8] / l3) / (4 * math.log(ad3[len(ad1) - 1])))
        else:
            mec3.append(1)

        if (l4 != 0 and ad4[len(ad4) - 1] != 0 and math.log(ad4[len(ad4) - 1]) != 0):
            mec4.append(math.log(df.iat[i, 11] / l4) / (4 * math.log(ad4[len(ad4) - 1])))
        else:
            mec4.append(1)
